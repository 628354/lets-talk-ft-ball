...
export default function TeamComparisionChartFour({GsGin2}) {
	useEffect(()=>{
		if (!GsGin2) return; // Handle cases where gainRate is not yet available
	  
		const root4 = am5.Root.new("chartdivGs2");
		...
		const dataWithIndex = GsGin2.map((item, index) => ({ ...item, index: index + 1 }));
  ...
  console.log(GsGin2);
    return (
        <div>
			...
			{
                GsGin2.length >0 ?( <h5>GS/g</h5>):( <h5>-1</h5>)
              }
                    <div className="main-charts">
					<div id="chartdivGs2" style={{ width: "100%", height: "300px" }} ></div>
                    </div>
                </div>
            </div>
