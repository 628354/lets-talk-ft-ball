...
const LeagueTable = () => {
  ...
  const handleSubmit = () => {
    ...
    axios.post('http://localhost:5000/addleague', formData)
      .then((response) => {
  ...
  const handleEditSubmit = () => {
    axios.post(`http://localhost:5000/updateLeague/${editLeague._id}`, {
  ...
  const handleDelete = (leagueId) => {
    axios.delete(`http://localhost:5000/removeLeague/${leagueId}`)
      .then(() => {
        const updatedLeagues = leaguedetails.filter((league) => league._id !== leagueId);
        setLeaguedetails(updatedLeagues);
      })
      .catch((error) => {
    ...
    {showEditForm && editLeague && (
      <div>
        ...
        <form>
          ...
          <button type="button" onClick={handleEditSubmit}>Submit Edit</button>
        </form>
      </div>
    )}
  </div>
